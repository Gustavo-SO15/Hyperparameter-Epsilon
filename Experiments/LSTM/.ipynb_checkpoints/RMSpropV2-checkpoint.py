...
def rmspropV2(params: List[Tensor],
            grads: List[Tensor],
...
def _single_tensor_rmspropV2(params: List[Tensor],
                           grads: List[Tensor],
                           square_avgs: List[Tensor],
                           grad_avgs: List[Tensor],
                           momentum_buffer_list: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        if (step==1) or (step%683==0):
            b = avg
            b = b.to('cpu')
            direct = 'rmsprop_LSTM1_vgrad/iter'+str(step)
            if not os.path.isdir(direct):
                os.mkdir(direct)
            np.save(direct+'/'+'vgrad'+str(i)+'_iter_'+str(step)+'.npy', b)
