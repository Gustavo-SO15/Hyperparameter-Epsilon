...
class AdamMomV2(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        super(AdamMomV2, self).__init__(params, defaults)
    ...
    def __setstate__(self, state):
        super(AdamMomV2, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for kk, p in enumerate(group['params']):
                ...
                if (state['step']==1) or (state['step']%683==0): #(state['step']<=201) or 
                    b = (1 - beta2)*grad_residual**2
                    b = b.to('cpu')
                    direct = 'adammom_LSTM1_vgrad/iter'+str(state['step'])
                ...
                if amsgrad:
                    ...
                    denom = (max_exp_avg_var.add_(group['eps']).sqrt() / math.sqrt(bias_correction2))
                else:
