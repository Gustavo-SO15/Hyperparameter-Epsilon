...
class T2Adam(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8, eps2=1e3,
        ...
        super(T2Adam, self).__init__(params, defaults)
    ...
    def __setstate__(self, state):
        super(T2Adam, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                porcentage = 0.5
               # if state['step']<=391: 
                aux3, _ = torch.sort(exp_avg_var.sqrt().flatten())

                n = int((1-porcentage)*aux3.size()[0])
                aux_d = aux3[n]/ math.sqrt(bias_correction2)  #grad_residual.median()
            ...
            #    aux2 = 10**math.floor(math.log10(aux2))
               ...
               # aux = group['eps']**(1-porcentage)*group['eps2']**(porcentage)
                aux = 10**math.floor(math.log10(aux))

                if  (state['step'] == 1) or ((state['step']% 391) == 0): 
