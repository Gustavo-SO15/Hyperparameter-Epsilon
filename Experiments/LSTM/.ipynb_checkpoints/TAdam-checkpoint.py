    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                bias_correction3 = 1 - 0.9999 ** state['step']
               ...
               #     print('max:'+str((grad**2).max()))
                ...
                if state['step']<= 663:  
               #     aux_d = torch.sqrt((grad**2).mean()*(grad**2).min())
               #     aux_d = 10**math.floor(math.log10(aux_d))
               #     group['eps'] = min(group['eps'], aux_d)            
               #     aux = group['eps']*bias_correction2       #group['eps']*bias_correction2*1000
                  #  print(group['eps'])
                    
               #     aux_d = (exp_avg_var).mean()
               #     aux_d = 10**math.floor(math.log10(aux_d))
               #     state['eps'] = min(state['eps'], aux_d) 
               #     aux = (state['eps'])*bias_correction2
               #     varx = torch.var(torch.abs(grad))
                #    print('eps: '+str(group['eps']))
                #    print('var: '+str(varx))
                #    print('mean: '+str(torch.mean(grad)))
                #    print('mean2: '+str(torch.mean(grad**2)))
                #    print('std: '+str(torch.std(grad)))
                            
                  #  group['eps'] = min(group['eps'],varx)
                    aux = group['eps']#*random.random()
                  #  print('mean1: '+str(torch.mean(grad.abs())))
                  #  print('std1: '+str(torch.std(grad.abs())))
                  #  print('var1: '+str(torch.var(grad.abs())))
                    mea=torch.mean(grad_residual)
                 #   print((grad_residual>=mea).sum()/grad_residual.nelement())
                 #   print((grad_residual>=1e-12).sum()/grad_residual.nelement())
                    print(grad_residual.nelement())
                    print('mean: '+str(torch.mean(grad_residual)))
                ...
                #if state['step']== 663:  
                #    print('mean: '+str((exp_avg_var).mean()))
                #    print('min: '+str((exp_avg_var).min()))
                #    print('max: '+str((exp_avg_var).max()))
                #    print('eps: '+str(group['eps']))
                #if state['step']<=  663:
                #    aux = 1e-13*bias_correction2       #group['eps']*bias_correction2*1000
                #else:
                #    aux = group['eps']*bias_correction2
