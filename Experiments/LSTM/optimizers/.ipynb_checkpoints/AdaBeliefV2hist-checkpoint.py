...
class AdaBeliefV2hist(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999),  eps_upper=0, eps_lower=1000,
        ...
        defaults = dict(lr=lr, betas=betas, eps_upper=eps_upper, eps_lower=eps_lower,
                        weight_decay=weight_decay, amsgrad=amsgrad)
        super(AdaBeliefV2hist, self).__init__(params, defaults)
    ...
    def __setstate__(self, state):
        super(AdaBeliefV2hist, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for kk, p in enumerate(group['params']):
                ...
                if (state['step']==391) : #or (state['step']%391==0): 
                    b = ((1-beta2)*exp_avg_var/bias_correction2).flatten()
                    b=b[b!=0]
                    aux3, _ = torch.sort(b)
                    
                    porcentage = 0.98
      
                    n_lower = int((1-porcentage)*aux3.size()[0])
                    n_upper = int(porcentage*aux3.size()[0])

                
                    group['eps_upper'] = max(group['eps_upper'], aux3[n_upper]) 
                    group['eps_lower'] = min(group['eps_lower'], aux3[n_lower]) 
                    
                    print(math.log10(group['eps_upper']))
                    print(math.log10(group['eps_lower']))
