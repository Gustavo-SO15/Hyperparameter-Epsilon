...
class Cal_eps_AdamMom(Optimizer):


    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), num_batches = 1, weight_decay=0, amsgrad=False, 
        ...
        super(Cal_eps_AdamMom, self).__init__(params, defaults)

        self.weight_decouple = weight_decouple
        self.rectify = rectify
        self.fixed_decay = fixed_decay
        if self.weight_decouple:
            print('Weight decoupling enabled in AdamMomentum')
            if self.fixed_decay:
                print('Weight decay fixed')
        if self.rectify:
            print('Rectification enabled in AdamMomentum')
        if amsgrad:
            print('AMS enabled in AdamMomentum')
    def __setstate__(self, state):
        super(Cal_eps_AdamMom, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:            
            ...
            for kk, p in enumerate(group['params']):
                ...
                grad = p.grad.data
                if grad.is_sparse:
                    raise RuntimeError('AdamMomentum does not support sparse gradients, please consider SparseAdam instead')
                amsgrad = group['amsgrad']
