...
class Cal_eps_RMSprop(Optimizer):

    def __init__(self, params, lr=1e-3, alpha=0.99, num_batches = 1, weight_decay=0, amsgrad=False, 
                 weight_decouple = False, fixed_decay=False, rectify = False ):
        if not 0.0 <= lr:
            raise ValueError("Invalid learning rate: {}".format(lr))
        if not 0.0 <= alpha < 1.0:
            raise ValueError("Invalid alpha parameter at index 0: {}".format(alpha))
            
        defaults = dict(lr=lr, alpha=alpha, num_batches=num_batches, weight_decay=weight_decay, amsgrad=amsgrad,  
                        eps_upper=0.0, eps_lower=3.4028235e+38)
        super(Cal_eps_RMSprop, self).__init__(params, defaults)

        self.weight_decouple = weight_decouple
        self.rectify = rectify
        self.fixed_decay = fixed_decay
        if self.weight_decouple:
            print('Weight decoupling enabled in RMSprop')
            if self.fixed_decay:
                print('Weight decay fixed')
        if self.rectify:
            print('Rectification enabled in RMSprop')
        if amsgrad:
            print('AMS enabled in RMSprop')
    def __setstate__(self, state):
        super(Cal_eps_RMSprop, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for kk, p in enumerate(group['params']):
                ...
                # State initialization
                if len(state) == 0:
                    state['rho_inf'] = 2.0 / (1.0 - alpha) - 1.0
                    state['step'] = 0
               ...
               # print(state['step'])
                ...
                exp_avg_var.mul_(alpha).add_(grad_residual, alpha = 1 - alpha)
                
                                
                if (state['step']==group['num_batches']) : 
                    b = (exp_avg_var).sqrt().flatten()
                    b=b[b!=0]
                    aux, _ = torch.sort(b)
                    ...
                    if (kk == num_params-1):
                        print('\n')
                        print('**********************************************')
                        print('Estimated search space for eps hyperparameter')
                        print('**********************************************')
