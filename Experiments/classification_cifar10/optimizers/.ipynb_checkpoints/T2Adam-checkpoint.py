...
class AdaBelief2(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        super(AdaBelief2, self).__init__(params, defaults)
    ...
    def __setstate__(self, state):
        super(AdaBelief2, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if state['step'] == 1:
                    print('value mean'+str((grad**2).flatten().median()))
                elif (state['step'] % 391) == 0:
                    print('value mean'+str((grad**2).flatten().median()))   
                elif state['step'] == 99*391:
                    print('value mean'+str((grad**2).flatten().median()))
                elif state['step'] == 105*391+1:
                    print('value mean'+str((grad**2).flatten().median()))
                elif state['step'] == 105*391:
                    print('value mean'+str((grad**2).flatten().median()))
