...
class AdaBelief2_inf(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        super(AdaBelief2_inf, self).__init__(params, defaults)
    ...
    def __setstate__(self, state):
        super(AdaBelief2_inf, self).__setstate__(state)
    ...
    def reset(self):
        for group in self.param_groups:
            for p in group['params']:
                ...
                state['global_inf'] = torch.zeros(1, device=torch.device('cuda')) 
                state['local_inf'] = torch.zeros(1, device=torch.device('cuda')) 
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                exp_avg, exp_avg_var, local_inf = state['exp_avg'], state['exp_avg_var'], state['local_inf']
                ...
                if self.weight_decouple:
                    ...
                    if group['weight_decay'] != 0:
                        ...
                        #grad.add_(torch.sign(p.data)/(1+10*torch.abs(p.data)),  alpha=group['eps'])

                beta3 = beta2
                bias_correction3 = 1 - beta3 ** state['step']
                            
                local_aux = grad.abs().max()               
                local_inf.mul_(beta3).add_(local_aux, alpha=1 - beta3)
                
                
               # if(state['step']<5):
               #     print(state['step'])
               #     print(state['global_inf'])
               #     print(local_inf)
                
                if  state['step'] == 1:
                    state['global_inf'] = local_inf/bias_correction3
                
                if (local_inf/bias_correction3 > state['global_inf']):
                    grad = torch.clamp(grad, min=-state['global_inf'], max=state['global_inf'])
                    #local_inf.add_(-local_aux, alpha=1 - beta3)
                   # local_inf.add_(bias_correction3*state['global_inf'], alpha=1 - beta3)    
                else:
                    state['global_inf'] = local_inf/bias_correction3
                ...
                if amsgrad:
                    ...
                    denom = (max_exp_avg_var.add_(eps_aux).sqrt() / math.sqrt(bias_correction2)).add_(group['eps'])
                else:
                    denom = (exp_avg_var.add_(eps_aux).sqrt() / math.sqrt(bias_correction2)).add_(group['eps'])
