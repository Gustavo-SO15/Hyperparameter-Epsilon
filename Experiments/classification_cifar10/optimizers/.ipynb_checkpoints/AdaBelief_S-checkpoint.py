...
class AdaBelief2_S(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        super(AdaBelief2_S, self).__init__(params, defaults)
    ...
    def __setstate__(self, state):
        super(AdaBelief2_S, self).__setstate__(state)
    ...
    def reset(self):
        for group in self.param_groups:
            for p in group['params']:
                ...
                state['exp_avg_var'] =  torch.zeros(1, dtype=p.dtype, layout=p.layout, device=p.device)
                if amsgrad:
                    # Maintains max of all exp. moving avg. of sq. grad. values
                    state['max_exp_avg_var'] =  torch.zeros(1, dtype=p.dtype, layout=p.layout, device=p.device)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                grad_residual = ((grad - exp_avg)**2).mean()
