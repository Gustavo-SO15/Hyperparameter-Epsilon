...
class AdamV2hist(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps_upper=0, eps_lower=1,
        ...
        for group in self.param_groups:
            for kk, p in enumerate(group['params']):
                ...
                if (state['step']==391) : #or (state['step']%391==0): 
                    ...
                    group['eps_upper'] = max(group['eps_upper'], aux3[n_lower]) 
                    group['eps_lower'] = min(group['eps_lower'], aux3[n_lower]) 
                    
                    

                eps_upper =  10**round(math.log10( group['eps_upper'] ))
 		eps_lower =  10**math.floor(math.log10( group['eps_lower'] ))
